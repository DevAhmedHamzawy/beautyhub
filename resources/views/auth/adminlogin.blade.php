							...
							<div class="card-sigin">
								...
								<div class="card-sigin">
									<div class="main-signup-header">
										<h2>مرحبا بك فى لوحة تحكم منصة موجود</h2>
										<h5 class="font-weight-semibold mb-4">من فضلك قم بتسجيل الدخول</h5>
										<form method="POST" action="{{ route('admin.login') }}">
											@csrf
											<div class="form-group">
												<label>البريد الالكترونى</label> <input placeholder="أدخل البريد الالكترونى" type="email" name="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email') }}">
												@if ($errors->has('email'))
											...
											<div class="form-group">
												<label>كلمة المرور</label> <input placeholder="أدخل كلمة المرور" type="password" name="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}">
