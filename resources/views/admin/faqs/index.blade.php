        ...
        <div class="col-xl-12">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ trans('faq.faqs') }}</li>
                </ol>
            </nav>
            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-title mg-b-0">{{ trans('faq.faqs') }}</h4>
                        <div class="d-flex">
                            <a class="btn btn-primary text-white" href="{{ route('admin.faqs.create') }}">
                                {{ trans('faq.add_new_faq') }}
                            </a>
                            &nbsp;&nbsp;
                            <a class="btn btn-primary text-white" href="{{ route('admin.faqs.trash') }}">
                                {{ trans('faq.trashed_faqs') }}
                            </a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="example" class="table key-buttons text-md-nowrap">
                            <thead>
                                <tr>
                                    <th class="border-bottom-0">{{ trans('faq.ar.question') }}</th>
                                    <th class="border-bottom-0">{{ trans('faq.en.question') }}</th>
                                    @canany(['view_faq', 'edit_faq', 'delete_faq', 'active_faq', 'restore_faq'])
                            ...
                            <tbody>
                                @foreach ($faqs as $faq)
                                    <tr>
                                        <td>{{ $faq->translate('ar')->question }}</td>
                                        <td>{{ $faq->translate('en')->question }}</td>
                                        @canany(['view_faq', 'edit_faq', 'delete_faq', 'restore_faq'])
                                            <td class="row pl-3">
                                                @can('edit_faq')
